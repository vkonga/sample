...
export async function requestEarlyAccess(
  ...
  try {
    ...
    if (!supabase) {
      throw new Error(
        'Database client could not be initialized. Please check server credentials.'
      );
    }
    ...
    if (dbError) {
      ...
      if (dbError.code === '23505') {
        return {
          ...
          error: 'This email address has already been registered. Thank you!',
        };
      }
    ...
    return {
      success: true,
      message: `Thank you, ${userName}! We've received your request and will be in touch soon.`,
    };
  } catch (error) {
